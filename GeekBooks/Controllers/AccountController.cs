    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult UpdateProfile(UserProfileViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var emailV = UserManager.FindByEmail(model.Email);
                if (emailV != null && (emailV.Id != currentUserId))
                {
                    UniqueEmail = false;
                }

                var userV = UserManager.FindByName(model.UserName);
                if (userV != null && (userV.Id != currentUserId))
                {
                ...
                if (UniqueEmail && UniqueUserName)
                {
                    ...
                    ApplicationUser updatedAccount = db.Users.Find(User.Identity.GetUserId());
                    updatedAccount.FirstName = model.FirstName;
                    updatedAccount.LastName = model.LastName;
                    updatedAccount.UserName = model.UserName;
                    updatedAccount.Email = model.Email;
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                return RedirectToAction("UserProfile", new { Message = "Your password has been changed." });
            }
            AddErrors(result);
            return View(model);
        }
